...
# ============================================================
# COLUMN NAMES (match Google Sheet headers) - GSRO
# ============================================================
COL_START = "start"
...
SERV_PREFIX = "Which services did you use? (Select all)/"
PROBE_PREFIX = "Probe list (tick if mentioned)/"
HELP_PREFIX = "What kind of legal help did you receive? (Select all)/"

COL_SERVICES_MULTI = "Which services did you use? (Select all)"
COL_WHEN_SERVICES = "When did you receive the service(s)?"
COL_INFO_IMPORTANT = "What information was shared about why a Tazkira is important?"
COL_STEPS_EXPLAINED = "What steps/process for obtaining a Tazkira were explained?"
COL_SUPPORT_WHERE = "Where did you receive most of the support?"

COL_LANG_UNDERSTAND = "Was the service provided in a language you understood?"
COL_BARRIERS = "Did you face any barriers to accessing services?"
BARRIERS_PREFIX = "Which barriers? (Select all)/"
COL_BARRIERS_MULTI = "Which barriers? (Select all)"
COL_REDUCE_BARRIERS = "How (if at all) did GSRO help reduce these barriers?"
COL_SAFE_ACCESS = "Did you feel safe while accessing the services?"

# Outcomes
COL_TAZKIRA_STATUS = "What is the current status of your Tazkira application?"
COL_TAZKIRA_RECEIVE_WHEN = "If received: when did you receive it?"
COL_FOLLOW_UP = "Did GSRO follow up after issuance?"
COL_CHANGED_AFTER = "What changed for you after receiving it?"
NOT_RECEIVED_PREFIX = "If not received: main reason(s)/"
COL_NOT_RECEIVED_MULTI = "If not received: main reason(s)"

COL_REFERRED = "Were you referred to another organisation/service for additional support?"
COL_REFERRED_HELPFUL = "If yes: what kind of support and was it helpful?"

# Safeguarding / AAP
COL_RESPECT = "Did you feel you were treated respectfully and fairly?"
COL_FEEDBACK_INFORMED = "Were you informed how to give feedback or make a complaint?"
FEEDBACK_CH_PREFIX = "If yes: which channel(s)?/"
COL_SAFE_PROVIDE = "Did you feel safe using the service / providing information?"
COL_CLOSING = "Closing: Anything else you want to share about GSRO’s support?"
COL_IFNO_WHY = "If no: why?"
COL_WHY = "Why?"
...
# ============================================================
# DATA UTILS (fast + safe) - copied from GWO style
# ============================================================
...
def render(spreadsheet_id: str, sheet_name: str):
    """Render the GSRO Beneficiary KII dashboard (GWO-style tabs/charts)."""

    inject_fonts()
    apply_ui()
    enable_altair_theme()

    glass_header("GSRO - Beneficiary KII Dashboard", "Modern * Altair charts * All records")
    ...
    # KPIs (same slots as GWO, mapped to GSRO)
    total = len(df)
    pct_female = float((df["_gender_norm"] == "Female").mean() * 100.0) if total else 0.0

    # Used at least one service (based on tick-all columns if present, else multi text)
    service_cols = [c for c in df.columns if str(c).startswith(SERV_PREFIX)]
    if service_cols:
        used_any = (df[service_cols].apply(lambda r: any(str(x).strip() for x in r), axis=1)).mean() * 100.0
    else:
        used_any = (_clean_text_series(_safe_col(df, COL_SERVICES_MULTI)) != "").mean() * 100.0

    # Received tazkira
    status = _clean_text_series(_safe_col(df, COL_TAZKIRA_STATUS)).str.lower()
    pct_received = float(status.str.contains("received").mean() * 100.0) if total else 0.0

    # Treated respectfully (proxy for satisfaction/helpful)
    pct_respect = float(df[COL_RESPECT].isin(["Yes", "Partly"]).mean() * 100.0) if (total and COL_RESPECT in df.columns) else 0.0

    kpi_row(
        [
            {"label": "Total KIIs", "value": f"{total:,}", "hint": "All records"},
            {"label": "Female respondents", "value": f"{pct_female:.1f}%", "hint": "Gender distribution"},
            {"label": "Used GSRO service(s)", "value": f"{used_any:.1f}%", "hint": "Any legal/awareness/hotline/etc."},
            {"label": "Received Tazkira", "value": f"{pct_received:.1f}%", "hint": "Based on application status"},
            {"label": "Treated respectfully", "value": f"{pct_respect:.1f}%", "hint": "Perceived fairness/respect"},
        ]
    )
    ...
    # TAB 3
    with tabs[2]:
        glass_header("Services, Training & Outcomes", "Service use, information recall, barriers, and documentation outcomes")

        c1, c2, c3 = st.columns(3, gap="large")
        with c1:
            services_any = pd.DataFrame(
                {"category": ["Used services", "No service recorded"], "count": [int(used_any/100*total), max(0, total-int(used_any/100*total))]}
            )
            if total:
                services_any["pct"] = services_any["count"] / max(1, int(services_any["count"].sum()))
            st.altair_chart(_donut(services_any, "Used GSRO Service(s)"), use_container_width=True)

        with c2:
            tstatus = _value_counts_df(_safe_col(df, COL_TAZKIRA_STATUS), top_n=4)
            st.altair_chart(_donut(tstatus, "Tazkira Application Status"), use_container_width=True)

        with c3:
            respect = _value_counts_df(_safe_col(df, COL_RESPECT), top_n=3)
            st.altair_chart(_donut(respect, "Treated Respectfully"), use_container_width=True)

        glass_divider()

        # "Topics covered" slot -> Probe list tick columns (what was remembered/explained)
        probe_items = _items_from_prefix(df, PROBE_PREFIX)
        rows = []
        for item in probe_items:
            col = f"{PROBE_PREFIX}{item}"
        ...
        if not probe_df.empty:
            probe_df["pct"] = probe_df["count"] / max(1, int(probe_df["count"].sum()))
            st.altair_chart(_barh(probe_df.sort_values("count", ascending=False), "Key Information Mentioned (Probe List)", max_rows=12, color_scheme="inferno"), use_container_width=True)
        else:
            # fallback to free-text fields
            info = _multiselect_counts(_safe_col(df, COL_STEPS_EXPLAINED))
            if not info.empty:
                st.altair_chart(_barh(info, "Steps / Process Explained (Top)", max_rows=12, color_scheme="inferno"), use_container_width=True)
            else:
                st.info("No probe-list or steps/process data detected.")

        glass_divider()

        # Bubble slot -> Services used (tick-all) as % selected vs count
        service_items = _items_from_prefix(df, SERV_PREFIX)
        if service_items:
            rows = []
            for item in service_items:
                col = f"{SERV_PREFIX}{item}"
                sel = _clean_text_series(_safe_col(df, col))
                cnt = int((sel != "").sum())
                pct = float(cnt / max(1, len(df)) * 100.0)
                rows.append({"item": item, "percent_received": pct, "avg_quantity": float(cnt)})
            svc_df = pd.DataFrame(rows).sort_values("percent_received", ascending=False)

            bubble = (
                alt.Chart(svc_df)
                .mark_circle(opacity=0.75, strokeWidth=1)
                .encode(
                    x=alt.X("item:N", title="Service type", sort=None),
                    y=alt.Y("percent_received:Q", title="% selected", scale=alt.Scale(domain=[0, 100])),
                    size=alt.Size("avg_quantity:Q", scale=alt.Scale(range=[120, 900]), legend=alt.Legend(title="Count")),
                    color=alt.Color("item:N", scale=alt.Scale(range=STATUS_PALETTE), legend=None),
                    tooltip=[
                        alt.Tooltip("item:N", title="Service"),
                        alt.Tooltip("percent_received:Q", title="% selected", format=".1f"),
                        alt.Tooltip("avg_quantity:Q", title="Count", format=".0f"),
                    ],
                )
                .properties(height=360, title="Services Used: % Selected vs Count")
            )
            st.altair_chart(bubble, use_container_width=True)
        else:
            st.info("Service tick-all columns not detected.")

        glass_divider()

        c4, c5 = st.columns(2, gap="large")
        with c4:
            barriers = _value_counts_df(_safe_col(df, COL_BARRIERS), top_n=3)
            st.altair_chart(_donut(barriers, "Faced Barriers to Accessing Services"), use_container_width=True)
        with c5:
            # Reasons/barriers
            reasons = _multiselect_counts(_safe_col(df, COL_BARRIERS_MULTI))
            for c in df.columns:
                cs = str(c)
                if cs.startswith(BARRIERS_PREFIX):
                    label = cs.replace(BARRIERS_PREFIX, "").strip()
                    sel = _clean_text_series(df[c])
                    cnt = int((sel != "").sum())
                    if cnt > 0:
                        reasons = pd.concat([reasons, pd.DataFrame([{"category": label, "count": cnt, "pct": 0.0}])], ignore_index=True)

            if reasons.empty:
                st.info("No barrier details captured.")
            else:
                ...
                st.altair_chart(_barh(reasons, "Which Barriers (Top)", max_rows=8), use_container_width=True)

        glass_divider()

        # 3 donut slot -> language understood, safe access, referred
        c6, c7, c8 = st.columns(3, gap="large")
        with c6:
            lang = _value_counts_df(_safe_col(df, COL_LANG_UNDERSTAND), top_n=3)
            st.altair_chart(_donut(lang, "Language Understood"), use_container_width=True)
        with c7:
            safe = _value_counts_df(_safe_col(df, COL_SAFE_ACCESS), top_n=3)
            st.altair_chart(_donut(safe, "Felt Safe Accessing Services"), use_container_width=True)
        with c8:
            ref = _value_counts_df(_safe_col(df, COL_REFERRED), top_n=3)
            st.altair_chart(_donut(ref, "Referred for Additional Support"), use_container_width=True)

        # Changes slot -> what changed after receiving tazkira (or progress)
        changes = _multiselect_counts(_safe_col(df, COL_CHANGED_AFTER))
        if changes.empty:
            changes = _value_counts_df(_safe_col(df, COL_CHANGED_AFTER), top_n=10)
        if not changes.empty:
            glass_divider()
            st.altair_chart(_barh(changes, "Changes After Receiving Tazkira", max_rows=10), use_container_width=True)

    # TAB 4
    with tabs[3]:
        glass_header("Safeguarding & Accountability", "Feedback awareness, safety, and open-ended responses")

        c1, c2, c3 = st.columns(3, gap="large")
        with c1:
            informed = _value_counts_df(_safe_col(df, COL_FEEDBACK_INFORMED), top_n=3)
            st.altair_chart(_donut(informed, "Informed How to Give Feedback/Complaint"), use_container_width=True)
        with c2:
            safe_use = _value_counts_df(_safe_col(df, COL_SAFE_PROVIDE), top_n=3)
            st.altair_chart(_donut(safe_use, "Felt Safe Providing Information"), use_container_width=True)
        with c3:
            respect = _value_counts_df(_safe_col(df, COL_RESPECT), top_n=3)
            st.altair_chart(_donut(respect, "Treated Respectfully/Fairly"), use_container_width=True)

        glass_divider()

        txt = _clean_text_series(_safe_col(df, COL_CLOSING))
        txt = txt[txt != ""]
        glass_header("Open-ended Feedback (Sample)", "Showing up to 15 examples")
        if txt.empty:
            st.success("No open-ended feedback recorded.")
        else:
            st.dataframe(txt.head(15).to_frame("Feedback"), use_container_width=True)

        # Optional: feedback channels (still under accountability)
        channels = _items_from_prefix(df, FEEDBACK_CH_PREFIX)
        if channels:
            rows = []
            for item in channels:
                col = f"{FEEDBACK_CH_PREFIX}{item}"
                sel = _clean_text_series(_safe_col(df, col))
                cnt = int((sel != "").sum())
                if cnt > 0:
                    rows.append({"category": item, "count": cnt})
            ...
            if not cdf.empty:
                cdf["pct"] = cdf["count"] / max(1, int(cdf["count"].sum()))
                glass_divider()
                st.altair_chart(_barh(cdf.sort_values("count", ascending=False), "Feedback / Complaint Channels Mentioned", max_rows=10), use_container_width=True)
